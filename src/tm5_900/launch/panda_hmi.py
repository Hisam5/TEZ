...
os.environ["QTWEBENGINE_CHROMIUM_FLAGS"] = (
    "--use-gl=swiftshader "          # Yazılımsal WebGL (en kritik satır)
    "--disable-gpu "
...
class ViewerFrame(QFrame):
    def __init__(self, title="TM5-900 · LIVE WEBOTS SCENE", parent=None):
        super().__init__(parent)
        self.setStyleSheet(f"ViewerFrame{{background:#040c14;border:1px solid {C['b']};border-radius:7px;}}")
        ly = QVBoxLayout(self); ly.setContentsMargins(0,0,0,0); ly.setSpacing(0)

        hdr = QFrame(); hdr.setFixedHeight(32)
        hdr.setStyleSheet("background:rgba(4,12,20,0.92);border-bottom:1px solid rgba(0,212,170,0.12);")
        hl = QHBoxLayout(hdr); hl.setContentsMargins(10,0,10,0)
        hl.addWidget(make_label(title, C["a"], 10, True))
        hl.addStretch()

        btn_reload = QPushButton("↻ RELOAD 3D")
        btn_reload.setStyleSheet(f"background:transparent; color:{C['t2']}; border:1px solid {C['t2']}; border-radius:3px; padding:2px 8px; font-size:9px;")
        hl.addWidget(btn_reload)
        ly.addWidget(hdr)

        # ── Windows IP'yi dinamik olarak bul ──────────────────────────────
        self._windows_ip = "127.0.0.1"
        try:
            cmd = "ip route show default | awk '{print $3}'"
            self._windows_ip = subprocess.check_output(
                cmd, shell=True).decode('utf-8').strip()
        except Exception as e:
            print(f"[HATA] Windows IP alınamadı: {e}")

        print(f"\n[BİLGİ] Webots WS hedefi: ws://{self._windows_ip}:1234\n")
        ...
        # Yazılımsal render için ekstra izinler
        self.browser.settings().setAttribute(QWebEngineSettings.Accelerated2dCanvasEnabled, True)

        # Sayfa tam yüklenince JS ile URL alanını doldur ve Connect'e bas
        self.browser.loadFinished.connect(self._on_load_finished)

        stream_url = f"http://{self._windows_ip}:1234/index.html"
        self.browser.setUrl(QUrl(stream_url))
    ...
    def _on_load_finished(self, ok: bool):
        if not ok:
            print("[HATA] Webots sayfası yüklenemedi — sunucu çalışıyor mu?")
            return

        ws_url = f"ws://{self._windows_ip}:1234"

        # Webots streaming viewer'ının DOM'undaki input alanını doldur
        # ve Connect düğmesine otomatik tıkla.
        js = f"""
        (function() {{
            // --- URL alanını bul ve doldur ---
            var inputs = document.querySelectorAll('input[type="text"], input:not([type])');
            for (var i = 0; i < inputs.length; i++) {{
                if (inputs[i].value.indexOf('localhost') !== -1 ||
                    inputs[i].value.indexOf('1234') !== -1 ||
                    inputs[i].value === '') {{
                    inputs[i].value = '{ws_url}';
                    // React / framework olaylarını da tetikle
            ...
            // --- Connect düğmesini bul ve tıkla ---
            // Webots sayfası bazen 300-500ms sonra hazır olur; kısa gecikme ekle
            setTimeout(function() {{
                var btns = document.querySelectorAll('button, input[type="button"], input[type="submit"]');
                for (var j = 0; j < btns.length; j++) {{
                    var txt = (btns[j].innerText || btns[j].value || '').toLowerCase();
                    if (txt.indexOf('connect') !== -1) {{
                        btns[j].click();
                        console.log('[HMI] Auto-connect tetiklendi → {ws_url}');
                        return;
                    }}
                }}
                // Buton metni farklıysa ilk butonu dene
                if (btns.length > 0) {{
                    btns[0].click();
                    console.log('[HMI] İlk buton tıklandı (fallback)');
                }}
            }}, 10000); 
        }})();
        """
    ...
    def _manual_reload(self):
        """Kullanıcı "↻ RELOAD 3D" butonuna basarsa sayfayı yenile ve yeniden bağlan."""
        self.browser.reload()
# ---------------------------------------------------------
